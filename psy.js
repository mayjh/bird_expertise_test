...
Data_Handler = (function () {
    ...
    //var total_trial = 1032;
    var data = { response: [], rt: [], properties: {} };
    var trial_num = 0;
    var too_slow = null;
    var response_flag = 0;
    
    //private variable: keeps track of which keys are down 
    var down = null;
    ...
    var now = (function () {
	  ...
	  function get_response(allowed_responses) {
        ...
        return new Promise(function (resolve_fn, reject_fn) {
           Data_Handler.response_flag = 0;
            start = now(); //start the timer
            var start2 = (new Date).getTime();
            if (Data_Handler.too_slow != null) {

              setTimeout(function(){
  			        if(Data_Handler.response_flag === 0){
  			        	var rt_null='NA';
  			        	var response_null='NA';
	                   data['rt'].push(rt_null);
	                   data['response'].push(response_null);
	                   $(document).off("keydown"); 
	                   resolve_fn();
        		       
  			        }else{resolve_fn();}
              },Data_Handler.too_slow)
            }
            

            //on keyup set down to null
            //this will allow the the get_response function
            //to resolve
            ...
            $(document).keydown(function (event) {
                ...
                var rt = now() - start; //get the reaction time
                var rt2 = (new Date).getTime() - start2;                
                ...
                if ($.inArray(response, allowed_responses) >= 0 || $.inArray(lower_case, allowed_responses)>=0) {
                  if (down === null) { //ignore keydown until a keyup event clears it
                    down = true;
                    Data_Handler.response_flag = 1;
...
function Img() {
    ...
    this.css_id = {
        ...
        image:'img',
        image_text_container: "image_text_container",
    };
    this.list = null;
    this.height = null;
    this.width = null;
    ...
    this.text = null;
    this.warning_time = 4000;
    this.warning_text = "RESPOND FASTER! \n \n \n \t Please still try to respond \n even if the picture was removed";
    this.css_warning_id = {warning:'warning',warning_container:'warning_container'};
...
Img.prototype = {
    ...
    show_text: function () {
        ...
        //plop the container back on the document
        var image_text_container = document.createElement("div");
        image_text_container.id = this.css_id.image_text_container;
        
        //add the container to the document
        $('#exp_container').append(image_text_container);
        //create a paragraph tag and message
        var message_para = document.createElement("p");
        var image_text = this.text[Data_Handler.trial_num];
        image_text = image_text.replace(/\t/g, "\n");
        var message = document.createTextNode(image_text);
        message_para.appendChild(message);
		
        //add message to the container
        image_text_container.appendChild(message_para);
    },
    ...
    run: function () {
        ...
        return new Promise(function (resolve_fn, reject_fn) {
            ...
            if (_this.time !== null && _this.time !== 'inf') {//timed stimulus
                ...
                // erase the image after _this.time 
                //var mytimer;
                mytimer = setTimeout(function () {
                    // if no response within 1 s, remove image first
                    Img.prototype.erase(_this.css_id.image_container);
                    
                    //then present warning screen, until response
                    var warning_container = $("<div>", { id: _this.css_warning_id.warning_container });
           			$("#exp_container").append(warning_container);
                    warning = $("<p>", { id: _this.css_warning_id.warning }).text(_this.warning_text);
	                $("#" + _this.css_warning_id.warning_container).append(warning);
	                // change the color of body and exp_container to be warning style
	                document.body.style.background=_this.warning_color;

	                
                }, _this.time);  
                

                // collect response then resolve
                Data_Handler.get_response(_this.allowed_responses)
                 .then(function () {
                 	 clearTimeout(mytimer); //clear timer

                     //erase image
                     var image_node = document.getElementById(_this.css_id.image_container);
                     if (image_node != null) {
                     Img.prototype.erase(_this.css_id.image_container); }
                     //erase warning
                     var warning_node = document.getElementById(_this.css_warning_id.warning_container);
                     if (warning_node != null) {
                     Img.prototype.erase(_this.css_warning_id.warning_container); 
                    // change the color of body and exp_container back to white
	                 document.body.style.background="white";
	                 }
	                 var image_text_node = document.getElementById(_this.css_id.image_text_container);
                     if (image_text_node != null) {
	                 Img.prototype.erase(_this.css_id.image_text_container); }
					// resolve after getting response               
                     resolve_fn();
                 });
...
function Feedback() {
    ...
    this.too_slow_text = 'Respond faster!'
...
Feedback.prototype = {
    ...
    run: function () {
        ...
        return new Promise(function (resolve_fn, reject_fn) {
            ...
            var response = Data_Handler.data.response.slice(-1)[0]; //get the most recent response
            var feedback_container = $("<div>", { id: _this.css_id.feedback_container });
            $("#exp_container").append(feedback_container);

            if (response === 'NA') {
                var feedback = $("<p>", { id: _this.css_id.feedback }).text(_this.too_slow_text);
                ...
                setTimeout(function () {
                    Img.prototype.erase(_this.css_id.feedback_container)
                    resolve_fn();
                }, _this.time);
            } else if (response == _this.correct_answers[Data_Handler.trial_num]) {
                //correct response
                //display "Correct!" 	
...
Blank_Screen.prototype = {
    ...
    run : function(){
        ...
        if (_this.time !== null) {
            ...
            if (_this.time[2]!==undefined) {
            	var blank_screen_time = _this.time[Data_Handler.trial_num];
            } else {
            	var blank_screen_time = _this.time;
            } 
            return new Promise(function (resolve_fn, reject_fn) {
                setTimeout(function () {
